...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    geom_bar() automatically counts up the number of rows as the y value, while for geom_col(), this has to be assigned manually. Source: <https://argoshare.is.ed.ac.uk/healthyr_book/bar-plots.html>
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    breaks determines the geometry of the histogram bars and the chart. the values inside seq() are the starting and end points of the interval of the data on the x-axis and the last value is the width of the bars.
...
6.  Run the code contained in the code chunk and fix any errors.
    ...
    Note: the legend is stopped from being displayed with show.legend = false, as the plot is barely visible otherwise.
